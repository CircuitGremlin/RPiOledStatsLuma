...
import luma.oled.device as Devices
...
class Pages:
    ...
    @staticmethod
    def setDevice(port=1, address=0x3C, driver="ssd1306"):
        """Set device"""
        if 'ch1115' in driver:
            Pages.device = Devices.ch1115(i2c(port=port, address=address), width=128, height=64)
        elif 'sh1106' in driver:
            Pages.device = Devices.sh1106(i2c(port=port, address=address), width=128, height=64)
        elif 'ssd1306' in driver:
            Pages.device = Devices.ssd1306(i2c(port=port, address=address), width=128, height=64)
        else:
            e = 'Display driver not supported yet! ch1115, sh1106 and ssd1306 are.'
            logging.error(e)
            sys.exit(e)
    ...
    def poweroff():
        ...
        with canvas(Pages.device) as DRAW:
            ...
            # Draw text
                ...
                # with canvas(DEVICE) as DRAW:
                    ...
                    if not os.path.ismount(mount_point):
                        ...
                        if DRAW.textlength(buffer, font=FONT) <= OLED_WIDTH:
            ...
            # Draw text
                ...
                # with canvas(DEVICE) as DRAW:
                    ...
                    if not Docker.get_docker_status():
                        ...
                        DRAW.rectangle((0, 0, OLED_WIDTH, OLED_HEIGHT),
                                       outline=0, fill=0)
